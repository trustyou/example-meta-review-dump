...
def process_meta_review(meta_review):
	...
	1. Translate meta_review["ty_id"] to your own hotel ID, dropping any
	unmapped hotels
	2. Traverse the JSON structure to find the data you are interested in,
	documented at http://api.trustyou.com/hotels/documentation.html
	3. Store data in your own database, to run reports, or serve them to a live
	website
...
def process_meta_review_dump(dest_folder):
	"""
	Look for all *.jsonl.gz files in the destination folder, and process each
	file line by line, calling process_meta_review on each JSON object.
	"""
	...
	for gz_path in sorted(gz_paths):
		...
		# Note that this is not the most performant way of unzipping files. In
		# production, consider using "gunzip" directly, and processing files in
		# parallel, e.g. via "xargs":
		# $ find $dest_folder -name "*.jsonl.gz" | xargs -n1 -P16 gunzip
		
		with gzip.open(gz_path) as jsonl_file:
			for line_bytes in jsonl_file:

				# Load a single line from the *.jsonl file. Each line represents
				# one meta-review. The same hotel will appear on ~20 consecutive
				# lines, once for every supported display language.

				# Identify the hotel with the "ty_id" property, and the language
				# with "lang".

				line = line_bytes.decode("utf-8")
				meta_review = json.loads(line)

				# Here, for demonstration purposes, we will ignore all languages
				# except English.
